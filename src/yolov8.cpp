...
YOLOV8::YOLOV8(const char *modelPath, const int32_t modelWidth, const int32_t modelHeight) : modelPath_(modelPath), modelWidth_(modelWidth), modelHeight_(modelHeight)
{
}
...
Result YOLOV8::ProcessInput(std::string testImgPath)
{
...
Result YOLOV8::Inference(std::vector<InferenceOutput> &inferOutputs)
{
...
Result YOLOV8::GetResult(std::vector<InferenceOutput> &inferOutputs, std::vector<BoundBox>& result,
                               std::string imagePath, bool release)
{
    ...
    while (boxes.size() != 0)
    {
        ...
        while (boxes.size() > index)
        {
            ...
            float xLeft = std::max(boxMax.x, boxCompare.x);
            float yTop = std::max(boxMax.y, boxCompare.y);
            float xRight = std::min(boxMax.x + boxMax.width, boxCompare.x + boxCompare.width);
            float yBottom = std::min(boxMax.y + boxMax.height, boxCompare.y + boxCompare.height);
            float width = std::max(0.0f, xRight - xLeft);
            float hight = std::max(0.0f, yBottom - yTop);
    ...
    for (size_t i = 0; i < result.size(); ++i)
    {
        ...
        std::string className = label[result[i].classIndex];
        std::string markString = to_string(result[i].score) + ":" + className;
    ...
    std::string savePath = "results/" + to_string(imageIndex) + ".jpg";
